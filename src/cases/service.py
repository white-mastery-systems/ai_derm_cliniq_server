...
async def get_systemic_symptoms(
    ...
    """
    Return the case-specific systemic symptom options for the Systemic Check screen.

    Generated by the AI mapper at the end of analysis and stored on the case.
    Falls back to the generic set if analysis hasn't completed or the column is empty.
    """
    ...
    if case.systemic_symptom_options:
        import json as _json
        try:
            return _json.loads(case.systemic_symptom_options)
        except Exception:
            pass

    # Fallback: generic symptom set (analysis not complete or column empty)
    return map_symptoms_from_differential(most_probable=None, differentials=[])
